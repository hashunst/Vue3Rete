...
export default {
  ...
  async mounted() {
    ...
    class NumComponent extends Component {
        ...
        constructor(){
            super("Number");
            this.data = {button: 'sava'}
        }

        builder(node) {
            var out1 = new Output('num', "Number", numSocket);

            return node.addControl(new NumControl(this.editor, 'num')).addOutput(out1).addControl(new ButtonControl(this.editor, 'button','تایید', ()=>{
              console.log(node)
              node.data.num = 4
              node.controls.get('num').setValue(4)
